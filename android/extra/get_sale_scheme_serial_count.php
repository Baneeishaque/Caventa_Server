...
$distinct_serial_sql = "SELECT DISTINCT `serial` FROM `ticket` WHERE `scheme`='$scheme' AND `insertion_date`='$date'";
...
while ($distinct_serial_row = mysqli_fetch_assoc($distinct_serial_result)) {
    $sum_serial_sql = "SELECT SUM(`count`) AS `count` FROM `ticket` WHERE  `scheme`='$scheme' AND `insertion_date`='$date' AND `serial`='" . $distinct_serial_row['serial'] . "'";
//    echo $sum_serial_sql;
