...
$status_sql = "SELECT CONCAT(works.name,\" ~Commission\") AS particulars, amount,work_profits.insertion_date_time FROM work_profits,sales_persons,works WHERE sales_persons.id=work_profits.sales_person_id AND works.id=work_id AND work_profits.sales_person_id!=1 UNION ALL SELECT CONCAT(works.name,\":~Advance:\",description) AS particulars, amount, works.insertion_date_time FROM work_advances,works WHERE works.id=work_advances.work_id UNION ALL SELECT CONCAT(works.name,\":~Expense:\",description) AS particulars, amount, works.insertion_date_time FROM work_expenses,works WHERE works.id=work_expenses.work_id UNION ALL SELECT CONCAT(\"~Investment:\",`description`) AS particulars, `amount`,`insertion_date_time` FROM `investments` UNION ALL SELECT CONCAT(\"~Other Expense:\",`description`) AS particulars, `amount`,`insertion_date_time` FROM `other_expenses` UNION ALL SELECT CONCAT(\"~Other Sale:\",`description`) AS particulars, `amount`,`insertion_date_time` FROM `other_sales` UNION ALL SELECT CONCAT( loans.description,  \":~Installment:\",  `receipt_number` ) AS particulars,  `payed_amount` AS `amount`, loan_installments.insertion_date_time FROM loan_installments, loans WHERE loans.id = loan_installments.loan_id UNION ALL SELECT CONCAT(\"~Loan:\",`description`) AS particulars, `loan_amount` AS `amount`,`insertion_date_time` FROM `loans` ORDER BY insertion_date_time DESC";
